...
class Conv3DTranspose(Conv3D):
    ...
    # Arguments
        filters: Integer, the dimensionality of the output space
            (i.e. the number of output filters in the convolution).
        kernel_size: An integer or tuple/list of 3 integers, specifying the
            width, height and depth of the 3D convolution window.
            Can be a single integer to specify the same value for
            all spatial dimensions.
        strides: An integer or tuple/list of 3 integers,
            specifying the strides of the convolution along the width and height.
    ...
    # Input shape
        5D tensor with shape:
        `(batch, channels, x_dim, y_dim, z_dim)` if data_format='channels_first'
        or 5D tensor with shape:
        `(batch, x_dim, y_dim, z_dim, channels)` if data_format='channels_last'.

    # Output shape
        5D tensor with shape:
        `(batch, filters, new_x_dim, new_y_dim, new_z_dim)` if data_format='channels_first'
        or 5D tensor with shape:
        `(batch, new_x_dim, new_y_dim, new_z_dim, filters)` if data_format='channels_last'.
        `x_dim`, `y_dim` and `z_dim` values might have changed due to padding.
